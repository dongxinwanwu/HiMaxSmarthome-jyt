...
typedef int (*TYTEST_F_W_CFG_HANDL)(char *auzkey, char *uuid, char *pid, int prodtest, char *ap_ssid, char *ap_pwd,char *psk);
...
typedef int (*TYTEST_F_GENERAL_BOOL_TEST_HANDL)(char *data, char *out_buf);
...
typedef struct {    
    ...
    TYTEST_F_WIFIBLE_RF_HANDL lte_rf_frame_cb;
    ...
    TYTEST_F_GENERAL_BOOL_TEST_HANDL general_booltest_cb; //71 general bool test
}TYTEST_FRAME_CBS_S;
...
int tuya_testframe_rep_key_event_repeat(int index, unsigned int key_index);
