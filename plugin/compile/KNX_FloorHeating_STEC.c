...
1     开关            switch            rw        bool
2     目标温度         temp_set         rw         value      数值范围: 50-350, 间距: 5, 倍数: 1, 单位: ℃
3     当前温度         temp_current     ro         value      数值范围: -20-80, 间距: 1, 倍数: 0, 单位: ℃
*/
#define DEBUG 1
...
int knx_dev_control(const ty_obj_dp_s *pObj, KnxObj_t *pCtrl) {
    ...
    switch (pCtrl->localPoint) {
        ...
        case HEAT_TEMP:
            pCtrl->value = ConvetTemp2Uint16(pObj->value.dp_value / 10.0);
...
int knx_dev_report(const KnxObj_t *pStatus, ty_obj_dp_s *pObj) {
    ...
    switch (pStatus->localPoint) {
        ...
        case HEAT_TEMP_STATUS:
            pObj->value.dp_value = ConvetUint162Temp(pStatus->value) * 10;
...
const KnxIf KNX_FloorHeating_STEC = {
