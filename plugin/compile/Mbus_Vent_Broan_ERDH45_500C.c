...
// 包含两款新风：ERDH24_350C、ERDH45_500C


/*
PID="6vbupeoxljq76aod"

DP ID 功能点名称 标识符 数据传输类型 数据类型 功能点属性
8 湿度设置 humidity_set 可下发可上报（rw） value "数值范围: 30-90,间距: 1, 倍数: 0,单位: %"
22 新风阀 fresh_air_valve 可下发可上报（rw） bool
28 风速	supply_fan_speed 可下发可上报（rw） enum 枚举值: low, high
39 开关	switch 可下发可上报（rw） bool
47 室内湿度 humidity_indoor 只上报（ro） value "数值范围: 0-100,间距: 1, 倍数: 0,单位: %"
        ...
        case 101: /*模式*/
            *((uint8_t *) (pMsg->pBuf + 6)) = 0x10;
            if (pObj->value.dp_enum == 0) {
                *((uint8_t *) (pMsg->pBuf + 7)) = 0;
            } else if (pObj->value.dp_enum == 1) {
                *((uint8_t *) (pMsg->pBuf + 7)) = 0x10;
            }
            break;
        default:
...
MbusObj_t **parse (uint8_t devId, UNUSED uint16_t regAddr, const void *pvMbusNode, const uint8_t *pBuf, uint16_t len) {
    ...
    ppMbusObj[FRESH_AIR_VALVE]->value = (pos[6] & 0x02) >> 1;
    ppMbusObj[FRESH_AIR_VALVE]->localPoint = VENT_VALVE;

    ppMbusObj[MODE]->value = (pos[6] & 0x08) >> 3;
    ppMbusObj[MODE]->localPoint = VENT_MODE;

    ppMbusObj[HUMIDITY_SET]->value = pos[8];
    ppMbusObj[HUMIDITY_SET]->localPoint = VENT_HUMI;

    ppMbusObj[HUMIDITY_INDOOR]->value = pos[9];
    ppMbusObj[HUMIDITY_INDOOR]->localPoint = VENT_ENV_HUMI;

    return ppMbusObj;
}

const MbusIf Mbus_Vent_Broan_ERDH45_500C = {
    .build = build,
    .parse = parse};
