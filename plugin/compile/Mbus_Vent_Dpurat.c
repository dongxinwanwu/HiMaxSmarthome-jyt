...
/* Dpurat 德普莱太 */
/*
 * producct id: ityiaeb3uzapmahe
DP    功能点名称        标识符            传输类型    数据类型    功能点属性
39     开关             switch          rw         bool
101    风速设置          fan_speed_enum  rw         enum       枚举值:  off, low, middle, high
*/

#define READ_CODE 0x3
#define WRITE_CODE 0x6

#define REG_ADDR 0x0B
#define WR_SWITCH_ADDR 0x01
#define WR_SPEED_ADDR 0x02
...
Msg_t *build(uint8_t devId, uint16_t regAddr, uint16_t groupAddr, const void *pvMbusNode, ty_obj_dp_s *pObj) {
    ...
    *((uint8_t *) pMsg->pBuf + 1) = WRITE_CODE;
    ...
    switch (pObj->dpid) {
        ...
        case 101: /* Speed */
            *((uint16_t *) (pMsg->pBuf + 2)) = htons(WR_SPEED_ADDR);
            *((uint16_t *) (pMsg->pBuf + 4)) = htons(pObj->value.dp_enum);
...
MbusObj_t **parse(uint8_t devId, uint16_t regAddr, const void *pvMbusNode, const uint8_t *pBuf, uint16_t len) {
    ...
    if (pos[1] == READ_CODE) {
        ...
        ppMbusObj[SPEED]->value = pos[6];
        logTrc ( "read mbus vent%d switch(%p)=%s speed(%p)=%d", devId, ppMbusObj[SWITCH],
                    (ppMbusObj[SWITCH]->value ? "on" : "off"), ppMbusObj[SPEED], ppMbusObj[SPEED]->value);
        ...
        if (reg == WR_SWITCH_ADDR) {
            ...
            logTrc ( "write mbus vent%d switch(%p)=%s, localPoint=%d", devId, ppMbusObj[0],
                        (ppMbusObj[0]->value ? "on" : "off"), ppMbusObj[0]->localPoint);
        } else if (reg == WR_SPEED_ADDR) {
            ppMbusObj[0]->localPoint = VENT_SPEED;
            ppMbusObj[0]->value = pos[5];
            logTrc ( "write mbus vent%d speed(%p)=%d, localPoint=%d", devId, ppMbusObj[0],
                        ppMbusObj[0]->value, ppMbusObj[0]->localPoint);
        } else {
            logErr ( "mbus plugin report: invalid command");
            return NULL;
        }
